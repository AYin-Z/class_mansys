...
/**
 * 同步 CloudBase 登录状态到后端
 * 在 CloudBase 认证成功后调用，获取后端 JWT token
 */
export const syncBackendAuth = async () => {
  try {
    const { data } = await auth.getSession();
    if (!data?.session?.user) {
      console.warn('CloudBase 会话不存在，跳过后端同步');
      return false;
    }
    ...
    if (hasBackendToken()) {
      console.log('后端 token 已存在，跳过同步');
      return true;
    }

    const result = await cloudBaseLoginAndStoreToken({
      uid,
      phone: user.phone || '',
      email: user.email || '',
    ...
    if (result.success && result.user) {
      ...
      setCurrentUser({
        ...
        isAdmin: result.user.role > 0,
        adminRole: result.user.role,
        phone: result.user.phone,
        email: result.user.email,
        avatarUrl: result.user.avatarUrl
      });
      console.log('后端认证同步成功');
      return true;
    }

    return false;
  } catch (error) {
    console.error('后端认证同步失败:', error);
    return false;
  }
};
