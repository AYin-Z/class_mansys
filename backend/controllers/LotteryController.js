...
class LotteryController {
  static async create(req, res) {
    try {
      ...
      await stampClassId('lotteries', id, scope.writeClassId);
  ...
  static async list(req, res) {
    try {
      const scope = await resolveScope(req.user);
      const lotteries = filterByClassScope(await Lottery.getAll(), scope);
  ...
  static async detail(req, res) {
    try {
      ...
      const scope = await resolveScope(req.user);
      if (!canAccessClassRecord(lottery, scope)) {
        return res.status(403).json({ success: false, error: '无权查看该抽奖' });
      }
