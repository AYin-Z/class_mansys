...
- 入口：「我的 → 用户手册」（路由 /pages/help/index），助手「微信 / MCP 接入」抽屉里也有直达链接；
- 内容源：frontend-v3/src/data/manual.ts（结构化章节 + 受众筛选 + 搜索），改手册只需改这一个文件；
- 章节：快速上手、角色与权限、请假与销假、通知与公告、作业、班费、积分、心理、相册、投票抽奖擂台、建议箱、中队视角（干部）、办事助手、微信接入、MCP 接入与部署、常见问题、数据与隐私、管理员运维速查（仅超管）；
- 受众：all / cadre / admin；页面按登录用户角色过滤并支持关键词搜索。
